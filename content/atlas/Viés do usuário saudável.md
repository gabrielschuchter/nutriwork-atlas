...
O conceito aparece na interpretação de estudos observacionais que avaliam associações entre padrões alimentares, suplementos nutricionais, rastreamento médico e desfechos de saúde. Indivíduos que seguem recomendações de saúde tendem também a apresentar maior escolaridade, melhor acesso aos serviços de saúde, menor prevalência de tabagismo e outros comportamentos favoráveis. Ignorar esse contexto pode levar à superestimação de benefícios atribuídos a exposições específicas.

## Base fisiológica ou epidemiológica

O mecanismo é epidemiológico e decorre da distribuição não aleatória de características entre grupos expostos e não expostos. Em estudos nutricionais, indivíduos que consomem mais frutas e hortaliças, por exemplo, frequentemente diferem em dezenas de variáveis relacionadas ao risco de doença. Mesmo modelos estatísticos extensamente ajustados podem não capturar integralmente essas diferenças devido a erro de medida, variáveis não observadas ou especificação inadequada dos modelos.

# **Heterogeneidade**

## **Definição**

Heterogeneidade é a variação entre estimativas, populações, intervenções, comparadores ou métodos. Em revisão sistemática, heterogeneidade clínica e metodológica descreve diferenças relevantes entre estudos; heterogeneidade estatística descreve dispersão de efeitos além do esperado apenas pelo erro amostral.

## **Contexto clínico**

Estudos podem divergir porque incluem pacientes com riscos basais, doses, durações e desfechos diferentes. Essa divergência pode indicar modificação real de efeito, viés, erro ou acaso. A decisão clínica deve verificar em qual população o resultado é aplicável.

I², tau² e teste Q são medidas ou testes com interpretações distintas. I² não informa direção, causa ou importância clínica da heterogeneidade.

## **Base fisiológica, bioquímica ou epidemiológica**

Modelos de efeitos fixos assumem um efeito comum sob certas condições; modelos de efeitos aleatórios incorporam distribuição de efeitos entre estudos. O efeito combinado e seu intervalo dependem do modelo, da variância entre estudos e do número de estudos.

Investigação usa características clínicas, [[Análise de Subgrupos]], meta-regressão e avaliação de [[Viés]]. A explicação deve ser guiada por hipótese, não por busca indiscriminada de um subgrupo com p baixo.

## **Limitações e controvérsias**

I² baixo pode ocorrer com poucos estudos e I² alto pode acompanhar diferenças pequenas quando precisão é grande. Teste de heterogeneidade tem baixo poder em algumas situações e não deve decidir sozinho o modelo.

Efeitos aparentemente consistentes podem compartilhar o mesmo viés. Efeitos divergentes não provam que intervenções sejam diferentes; avaliação de desenho e desfechos é necessária.

## **Conceitos relacionados**

- [[Gráfico de floresta]]
- [[Análise de Subgrupos]]
- [[Teste de interação]]
- [[Viés]]
- [[Risco Relativo]]
- [[Razão de Chances]]
- [[Intervalo de Confiança]]
- [[Clinicamente relevante]]
- [[Heterogeneidade de efeito do tratamento]]

## **Referências de base**

- Cochrane Collaboration. *Cochrane Handbook for Systematic Reviews of Interventions*, Chapter 10. https://training.cochrane.org/handbook/current/chapter-10
- Cochrane Collaboration. *Cochrane Handbook for Systematic Reviews of Interventions*, Chapter 6. https://training.cochrane.org/handbook/current/chapter-06
- Higgins JPT, Thompson SG. *Quantifying heterogeneity in a meta-analysis*. Statistics in Medicine. https://doi.org/10.1002/sim.1186
